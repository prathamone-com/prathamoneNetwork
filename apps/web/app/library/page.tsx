    ...
    <main style={{ minHeight: '100dvh', padding: '6rem 3.5rem', background: 'var(--deep)', color: 'var(--text)', position: 'relative', zIndex: 1 }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '3rem' }}>
          <h1 style={{ fontSize: '2.5rem', fontWeight: 300, letterSpacing: '0.05em' }}>Scholar Library</h1>
        ...
        <div className="card-luxury" style={{ padding: '3rem', textAlign: 'center' }}>
          ...
          <h2 style={{ fontSize: '1.8rem', fontWeight: 400, color: 'var(--gold-light)', marginBottom: '1rem' }}>
            Unlimited Curated Knowledge
          </h2>
          <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', maxWidth: '600px', margin: '0 auto 2.5rem', lineHeight: '1.6' }}>
            Access our scholar repository of MSBSHSE Grade 10-12 curricula, high-resolution textbooks, and supplementary AI-generated study aids.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.5rem' }}>
            {['Physics', 'Chemistry', 'Biology', 'Mathematics'].map(subject => (
              <div key={subject} className="card-luxury" style={{ padding: '1.5rem', background: 'rgba(255,255,255,0.02)' }}>
                <h3 style={{ fontSize: '1.1rem', color: 'var(--gold)' }}>{subject}</h3>
                <p style={{ fontSize: '0.8rem', opacity: 0.6 }}>12 Modules • 48 Books</p>
              </div>
            ))}
          </div>
        </div>
